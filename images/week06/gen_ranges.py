...
for i, v in enumerate([16, 8, 4, 2, 1][::-1]):
    parts.append(f'<text x="{lx+20}" y="{306-i*56}" font-family="{FONT}" font-size="23" fill="{INK}">{v}{" — 질문 " + str(i) + "번" if i else " — 질문 0번"}</text>')
parts += caption(lx + 90, 356, ["후보 ×2 = 질문 +1"], 22)
W, H = 880, 392
svg = (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">\n'
       + "\n".join(parts) + "\n</svg>\n")
...
# ---------- p3: 1~100 띠 ----------
parts = []
parts += strip(80, 80, 560, 46, "1", "100")
parts.append(f'<text x="360" y="48" text-anchor="middle" font-family="{FONT}" font-size="27" font-weight="700" fill="{INK}">이번엔 후보 100개</text>')
parts += caption(360, 190, ["소문: “7번이면 반드시 맞힐 수 있다” — 정말일까?", "홀수 개가 남으면 어떻게 가르는 게 최선일까?"], 22)
W, H = 740, 246
svg = (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">\n'
       + "\n".join(parts) + "\n</svg>\n")
...
for i, n in enumerate(chain):
    ...
    if i < len(chain) - 1:
        parts += arrow_r(x + bw + 3, x + bw + 27, y + bh / 2)
parts += caption(490, 60, ["최악의 경우 후보 수 — 일곱 계단이면 1"], 23)
parts += caption(490, 210, ["거꾸로 두 배의 사다리: 2 · 4 · 8 · 16 · 32 · 64 · 128",
